...
fn main() {
    ...
    for k in 1..=NUM_ITERATIONS {
        ...
        let gradient: Vec<f32> = gradient_func(&x);
        ...
        println!("Iteration number {}: W = {:?}, f(x) = {:.4}", k, w, func(&x));
    }
}

/// Определение функции f(x) над которой экспериментируем
fn func(x: &[f32]) -> f32 {
    x[0].powi(2) + 2.0 * x[1].powi(2) + 3.0 * x[2].powi(2) + x[0] * x[1] + x[1] * x[2] + 5.0
}

/// Вычисление градиента функции f(x) (берем частные производные)
fn gradient_func(x: &[f32]) -> Vec<f32> {
...
fn gradient_f_w(w: &[f32], x: &[f32]) -> Vec<f32> {
    ...
    w.iter().zip(&gradient).map(|(w, g)| w * g).collect::<Vec<_>>()
}
...
/// Генерация ядра (чем больше отклонение, тем больше сглаживание)
...
mod tests {
    ...
    fn test_conv_grad() {
        ...
        for (res, exp) in result.iter().zip(expected.iter()) {
            assert!((res - exp).abs() < 1e-6, "Got {res}, expected {exp}");
        }
    }

    // Проверяем, что размер возвращаемого ядра соответствует заданному размеру, и возвращаемые значения совпадают с референсными
    #[test]
    fn test_kernel() {
        let size: usize = 5;
        let sigma: f32 = 1.0;
